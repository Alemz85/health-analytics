...
- `recovery_plan_items(id, injury_id, name, kind, start_week, weekly_target, green_min, yellow_min, target_sets, target_reps, steps, note, active, exercise_id, created_at, updated_at)` — the canonical plan used by both Injury and Gym. `start_week` is the cumulative plan week in which the item first becomes adherence-accountable. Every `kind='exercise'` item is catalog-linked (`exercise_id` set) with `target_sets`/`target_reps` — that's what makes a session built from the plan real, editable Gym rows instead of a locked block. A composite/multi-movement routine ALSO carries structured `steps`, an array of `{name, sets, reps, duration_seconds, distance_m, per_side, note}` entries, alongside its catalog link — so stretches and mobility work stay tabular and readable as plan-item detail rather than being buried in prose, without giving up the catalog link every exercise item now requires.
...
python3 injuries.py note-update <note_id> --note ".."
python3 injuries.py note-remove <note_id>
...
python3 injuries.py plan-add <injury_id> --name ".." [--kind exercise|habit|constraint|activity] [--start-week 1-52] [--target 1-14] [--green-min 1-14] [--yellow-min 1-14] [--target-sets 1-20] [--target-reps 1-100] [--note ".."] --exercise "<catalog name>" [--create [--body-part chest|back|shoulders|arms|legs|core|"full body"]]
python3 injuries.py plan-update <item_id> [--name ".."] [--kind ..] [--start-week 1-52] [--target 1-14|none] [--green-min 1-14|none] [--yellow-min 1-14|none] [--target-sets 1-20|none] [--target-reps 1-100|none] [--steps-file steps.json|none] [--note ".."] [--active true|false] --exercise "<catalog name>"|none [--create [--body-part ..]]
...
### `note` is append-journal, not the app's same-day merge — correct rows by exact ID
...
Every successful `note` command prints the created note ID. `notes` and `show` also display each note's ID, so corrections are always targeted at one explicit row rather than inferred from the date or text.

Consequences for CORRECTING (not adding to) a same-day entry:

- First run `injuries.py show <injury_id>` or `injuries.py notes <injury_id>` and identify the exact note ID. Never infer a target from a matching date: multiple real events can share a date.
- For a wrong note text, run `injuries.py note-update <note_id> --note "corrected text"`. For a wholly mistaken note, run `injuries.py note-remove <note_id>`. Both commands fail if that exact row is absent; re-list rather than guessing another ID.
...
- **Every `kind='exercise'` item is catalog-backed — no exceptions.** Set `exercise` to a catalog name and provide `target_sets`/`target_reps`; that prescription loads directly into the Gym log, and a session built from this plan is fully editable (real sets the user can adjust, add to, or drop), not a locked block of prose. There is no more "off-catalog exercise" escape hatch — a rehab movement that only existed as a mobility routine still needs a real `exercises` row.
  - If the exact catalog name already exists (check with a `db.py` SELECT or let `plan-apply`/`plan-add`/`plan-update` tell you via its near-match error), just link it — no `create` needed.
  - If it genuinely doesn't exist yet, opt in explicitly: pass `"create": true` (contract) or `--create` (`plan-add`/`plan-update`) alongside `exercise`, plus a `body_part` (contract) / `--body-part` from the vocabulary (`chest`/`back`/`shoulders`/`arms`/`legs`/`core`/`full body`) when you can infer one. This mints a new `source='user'` catalog row and links it — mirrors exactly how `gym.py log`'s per-set `"create": true` behaves. `create` is a deliberate per-item opt-in, never a default: a typo in `exercise` without `create` aborts with near-matches instead of silently spawning a duplicate catalog row (e.g. "Heel walks" vs the existing "Heel Walk" — check near-matches before opting into create).
  - **A composite/multi-movement routine (e.g. a 4-step ankle mobility sequence) still gets ONE catalog link** — usually a sensible parent name for the whole routine (create it if nothing fits) — plus a single summary `target_sets`/`target_reps` for the loggable block, AND non-empty structured `steps` carrying the full per-movement detail (each step: a name and exactly one dose measure — `reps`, `duration_seconds`, or `distance_m`; `sets`, `per_side`, `note` optional). `steps` is plan-item detail, not a substitute for the catalog link, and now may coexist with `exercise`/`target_sets`/`target_reps` on the same item — the old "linked exercise cannot also carry steps" restriction is gone. Never encode a mobility sequence only inside the parent note.
