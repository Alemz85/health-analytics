...
- **Weekly-target rulebook** (the app scores adherence with fixed thresholds — design targets to fit it): every `exercise` and `activity` item meant to be done regularly MUST carry a `weekly_target` (1-14). The app rates each item's week as: red = 0 done, yellow = below 75% of target, green = >= 75% of target (PROVISIONAL blanket rule — per-item thresholds you assign case-by-case are planned; see TODO.md #4 in the repo). Set targets as realistic sustainable minimums so green is genuinely reachable in a normal week — never aspirational maximums (a 7/wk target where 5 is realistic guarantees permanent yellow and defeats the signal). Only `exercise` items count toward the overall adherence score; `activity` items are cleared training, tracked but unscored. `constraint` items never have targets.
