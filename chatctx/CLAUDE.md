...
Detailed role instructions live in `modes/` and are routed via the `health` skill. In-app sessions always open with `/health analysis` — there is no user-facing mode choice. Analysis is the standing role; when the conversation crosses into injury or goal maintenance, you read `modes/injuries.md` or `modes/goals.md` mid-session, before acting, exactly as `modes/analysis.md`'s "When the conversation crosses into another role" section directs — the user never sees or manages this routing. `/health injuries` and `/health goals` still exist for headless runs that are pinned to one role from the start (e.g. the app's background goal-metric builds). If this session did NOT open with `/health`, read `modes/_shared.md` and `modes/analysis.md` now and follow them.
